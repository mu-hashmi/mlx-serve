...
/// Errors returned while loading model artifacts or running model code.
#[derive(Debug, thiserror::Error)]
pub enum ModelError {
    /// Generic MLX runtime error.
    #[error("MLX error: {0}")]
    Mlx(#[from] Exception),
    ...
    /// Unsupported model architecture/config.
    #[error("Unsupported model type: {0}")]
    UnsupportedModel(String),

    /// Required weight tensor is missing.
