//! Model implementations and weight-loading helpers for MLX serving.
//!
//! This crate exposes architecture-specific model types plus a small
//! architecture-agnostic wrapper (`AnyModel`) used by the engine.

/// KV cache primitives shared by model implementations.
pub mod cache;
/// Error types for model loading and forward passes.
pub mod error;
/// Qwen3-Next hybrid SSM/attention model implementation.
pub mod qwen3_next;
/// Model architecture registry and detection helpers.
pub mod registry;
/// Transformer-family model implementation (Llama/Mistral/Qwen-style).
pub mod transformer;
/// Utility helpers for tensor and shape handling.
pub mod utils;
...
impl AnyModel {
    /// Run a forward pass for the wrapped model using the matching cache type.
    pub fn forward(
        &mut self,
        inputs: &Array,
        mask: Option<&Array>,
    ...
    /// Create an empty cache value for the wrapped model architecture.
