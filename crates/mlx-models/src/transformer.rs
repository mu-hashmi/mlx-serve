...
pub fn load_model(model_dir: impl AsRef<Path>) -> Result<Model, ModelError> {
    ...
    if quantization.is_some() {
        load_quantized_embed_tokens(&mut model, model_path)?;
    }
...
fn load_quantized_embed_tokens(model: &mut Model, model_path: &Path) -> Result<(), ModelError> {
    let embed_tokens = match &mut model.model.embed_tokens {
        MaybeQuantized::Quantized(embed) => embed,
        MaybeQuantized::Original(_) => {
            return Ok(());
        }
    };

    let mut embed_weight = None;
    let mut embed_scales = None;
    let mut embed_biases = None;
    ...
    for file_path in crate::collect_safetensors_files(model_path)? {
        ...
        for (name, tensor) in loaded {
            match name.as_str() {
                "model.embed_tokens.weight" => embed_weight = Some(tensor),
                "model.embed_tokens.scales" => embed_scales = Some(tensor),
                "model.embed_tokens.biases" => embed_biases = Some(tensor),
                _ => {}
            }
        }
    ...
    let embed_weight = embed_weight.ok_or_else(|| {
        ModelError::MissingWeight("model.embed_tokens.weight not found".to_owned())
    })?;
    let embed_scales = embed_scales.ok_or_else(|| {
        ModelError::MissingWeight("model.embed_tokens.scales not found".to_owned())
    })?;
    let embed_biases = embed_biases.ok_or_else(|| {
        ModelError::MissingWeight("model.embed_tokens.biases not found".to_owned())
    })?;

    embed_tokens.inner.weight.value = embed_weight;
    embed_tokens.scales.value = embed_scales;
    embed_tokens.biases.value = embed_biases;
